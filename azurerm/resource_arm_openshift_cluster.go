	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"service_principal": {
				...
				Set: resourceOpenShiftClusterServicePrincipalProfileHash,
			},
...
func resourceArmOpenShiftClusterCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if requireResourcesToBeImported && d.IsNewResource() {
		...
		if err != nil && !utils.ResponseWasNotFound(existing.Response) {
			return fmt.Errorf("Error checking for presence of existing Managed OpenShift Cluster %q (Resource Group %q): %s", name, resourceGroupName, err)
		}
	...
	linuxProfile := expandOpenShiftClusterLinuxProfile(d)
	masterProfiles := expandOpenShiftClusterMasterPoolProfiles(d)
	agentProfiles := expandOpenShiftClusterAgentPoolProfiles(d)
	servicePrincipalProfile := expandAzureRmOpenShiftClusterServicePrincipal(d)
	networkProfile := expandOpenShiftClusterNetworkProfile(d)
	addonProfiles := expandOpenShiftClusterAddonProfiles(d)
	tags := d.Get("tags").(map[string]interface{})
	rbacRaw := d.Get("role_based_access_control").([]interface{})
	rbacEnabled, azureADProfile := expandOpenShiftClusterRoleBasedAccessControl(rbacRaw, tenantId)
	...
	parameters := containerservice.OpenShiftManagedCluster{
		...
		OpenShiftManagedClusterProperties: &containerservice.OpenShiftManagedClusterProperties{
			...
			MasterPoolProfile: &masterProfiles,
	...
	if err != nil {
		return fmt.Errorf("Error creating/updating Managed Kubernetes Cluster %q (Resource Group %q): %+v", name, resourceGroupName, err)
	}

	if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("Error waiting for completion of Managed Kubernetes Cluster %q (Resource Group %q): %+v", name, resourceGroupName, err)
	}

	read, err := client.Get(ctx, resourceGroupName, name)
	if err != nil {
		return fmt.Errorf("Error retrieving Managed Kubernetes Cluster %q (Resource Group %q): %+v", name, resourceGroupName, err)
	}

	if read.ID == nil {
		return fmt.Errorf("Cannot read ID for Managed Kubernetes Cluster %q (Resource Group %q)", name, resourceGroupName)
	}
...
func resourceArmOpenShiftClusterRead(d *schema.ResourceData, meta interface{}) error {
	...
	if props := resp.ManagedClusterProperties; props != nil {
		...
		addonProfiles := flattenOpenShiftClusterAddonProfiles(props.AddonProfiles)
		...
		linuxProfile := flattenOpenShiftClusterLinuxProfile(props.LinuxProfile)
		...
		roleBasedAccessControl := flattenOpenShiftClusterRoleBasedAccessControl(props, d)
		...
		servicePrincipal := flattenAzureRmOpenShiftClusterServicePrincipalProfile(props.ServicePrincipalProfile)
		...
		if props.AadProfile != nil {
			...
			adminKubeConfigRaw, adminKubeConfig := flattenOpenShiftClusterAccessProfile(adminProfile)
	...
	kubeConfigRaw, kubeConfig := flattenOpenShiftClusterAccessProfile(profile)
...
func flattenOpenShiftClusterAccessProfile(profile containerservice.ManagedClusterAccessProfile) (*string, []interface{}) {
	if accessProfile := profile.AccessProfile; accessProfile != nil {
		if kubeConfigRaw := accessProfile.KubeConfig; kubeConfigRaw != nil {
			...
			if strings.Contains(rawConfig, "apiserver-id:") {
				...
				flattenedKubeConfig = flattenOpenShiftClusterKubeConfigAAD(*kubeConfigAAD)
			} else {
				...
				flattenedKubeConfig = flattenOpenShiftClusterKubeConfig(*kubeConfig)
			}
...
func expandOpenShiftClusterAddonProfiles(d *schema.ResourceData) map[string]*containerservice.ManagedClusterAddonProfile {
...
func flattenOpenShiftClusterAddonProfiles(profile map[string]*containerservice.ManagedClusterAddonProfile) []interface{} {
...
func expandOpenShiftClusterMasterPoolProfiles(d *schema.ResourceData) []containerservice.OpenShiftManagedClusterMasterPoolProfile {
	configs := d.Get("master_pool_profile").([]interface{})
	profiles := make([]containerservice.OpenShiftManagedClusterMasterPoolProfile, 0)
...
func flattenOpenShiftClusterAgentPoolProfiles(profiles *[]containerservice.ManagedClusterAgentPoolProfile, fqdn *string) []interface{} {
	if profiles == nil {
		return []interface{}{}
	}
...
func expandOpenShiftClusterLinuxProfile(d *schema.ResourceData) *containerservice.LinuxProfile {
...
func flattenOpenShiftClusterLinuxProfile(profile *containerservice.LinuxProfile) []interface{} {
	if profile == nil {
		return []interface{}{}
	}
...
func expandOpenShiftClusterNetworkProfile(d *schema.ResourceData) *containerservice.NetworkProfileType {
...
func flattenOpenShiftClusterNetworkProfile(profile *containerservice.NetworkProfileType) []interface{} {
	if profile == nil {
		return []interface{}{}
	}
...
func expandOpenShiftClusterRoleBasedAccessControl(input []interface{}, providerTenantId string) (bool, *containerservice.ManagedClusterAADProfile) {
	if len(input) == 0 {
		return false, nil
	}
...
func flattenOpenShiftClusterRoleBasedAccessControl(input *containerservice.ManagedClusterProperties, d *schema.ResourceData) []interface{} {
	rbacEnabled := false
...
func expandAzureRmOpenShiftClusterServicePrincipal(d *schema.ResourceData) *containerservice.ManagedClusterServicePrincipalProfile {
...
func flattenAzureRmOpenShiftClusterServicePrincipalProfile(profile *containerservice.ManagedClusterServicePrincipalProfile) *schema.Set {
	if profile == nil {
		return nil
	}

	servicePrincipalProfiles := &schema.Set{
		F: resourceOpenShiftClusterServicePrincipalProfileHash,
	}
...
func resourceOpenShiftClusterServicePrincipalProfileHash(v interface{}) int {
...
func flattenOpenShiftClusterKubeConfig(config kubernetes.KubeConfig) []interface{} {
	values := make(map[string]interface{})
...
func flattenOpenShiftClusterKubeConfigAAD(config kubernetes.KubeConfigAAD) []interface{} {
	values := make(map[string]interface{})
